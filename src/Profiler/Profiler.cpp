...
std::pair<std::vector<boost::filesystem::path>, std::vector<boost::filesystem::path>> Profiler::profile(const boost::filesystem::path & path)
{
	...
	{
		std::string message = (boost::format("[profiling : \"%1%\"] invalid profiling root directory") % path.string()).str();
	...
	{
		std::string message = (boost::format("[profiling : \"%1%\"] profiling root directory don't have read and write rights") % root_directory.get_path().string()).str();
		spdlog::error(message);
		throw std::runtime_error(message);
	}

	spdlog::info("[profiling : \"{}\"] start of profiling", root_directory.get_path().string());
	...
	spdlog::info("[profiling : \"{}\"] start of profiling iteration", root_directory.get_path().string());
	...
	while(node_it != node_end)
	{
		...
		if(Node::get_guessed_type(node_it->path()) == Node::ACCES_DENIED)
		{
			...
			node_it.no_push();
			spdlog::warn("[profiling : \"{}\"] can't add node path \"{}\" to the profile : {}", root_directory.get_path().string(), node_it->path().string(), "Permission denied");
		}
		...
		if(Node::get_guessed_type(node_it->path()) == Node::NODE)
		{
			unvalid_paths.push_back(node_it->path());
			node_it.no_push();
			spdlog::warn("[profiling : \"{}\"] can't add node path \"{}\" to the profile : {}", root_directory.get_path().string(), node_it->path().string(), "Undefined node type");
		}
		...
		else if(Node::get_guessed_type(node_it->path()) == Node::SYMLINK)
		{
			unvalid_paths.push_back(node_it->path());
			node_it.no_push();
			spdlog::warn("[profiling : \"{}\"] can't add symlink file path \"{}\" to the profile : {}", root_directory.get_path().string(), node_it->path().string(), "Symlink not supported");
		}
		...
		else if(Node::get_guessed_type(node_it->path()) == Node::FILE)
		{
			...
			if(file.get_has_permissions(Node::USER, {Node::READ, Node::WRITE}))
			{
				...
				spdlog::info("[profiling : \"{}\"] add file path \"{}\" to the profile", root_directory.get_path().string(), file.get_path().string());
			}
			...
			else
			{
				...
				node_it.no_push();
				spdlog::warn("[profiling : \"{}\"] can't add file path \"{}\" to the profile : {}", root_directory.get_path().string(), file.get_path().string(), "Permission denied");
			}
		}
		...
		else if(Node::get_guessed_type(node_it->path()) == Node::DIRECTORY)
		{
			...
			if(ec_tmp == no_error)
			{
				...
				spdlog::info("[profiling : \"{}\"] add directory path \"{}\" to the profile", root_directory.get_path().string(), directory.get_path().string());
			}
			//can access to directory child nodes
			else
			{
				unvalid_paths.push_back(directory.get_path());
				spdlog::warn("[profiling : \"{}\"] can't add directory path \"{}\" to the profile : {}", root_directory.get_path().string(), directory.get_path().string(), ec_tmp.message());
	...
	spdlog::info("[profiling : \"{}\"] end of profiling iteration ({} valid and {} unvalid node(s) found in {} second(s))", root_directory.get_path().string(), valid_paths.size(), unvalid_paths.size(), elapsed_seconds_profiling_iteration.count());

	spdlog::info("[profiling : \"{}\"] start of duplicated path removal", root_directory.get_path().string());
	...
	spdlog::info("[profiling : \"{}\"] end of duplicated path removal ({} path(s) removed in {} second(s))", root_directory.get_path().string(), nbr_removed_path, elapsed_seconds_duplicated_removal.count());
	...
	spdlog::info("[profiling : \"{}\"] end of profiling (done in {} second(s))", root_directory.get_path().string(), elapsed_seconds_profiling.count());
